...
`async` не принимает `ref`, `ref readonly`, `in` и `out` (**CS1988**): машина состояний не удерживает ссылку между `await`. Асинхронная станция читает вагоны по значению и возвращает данные. Параметр `in` библиотека не использует. На `.ServiceStation` состав не меняется (**TOP015**–**TOP017**): хвост уже ждёт этот набор. Запасной путь без вагонов — `(RedSignal red, CargoManifest manifest)` и `manifest.LoadWagon`.
...
| `TOP019` | Error | Имя `ref readonly` присутствует в возврате |
